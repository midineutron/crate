...
// Gray palette for dead / NO SIGNAL channels.
const GRAY = { main: '#c8c8c8', mid: '#6a6a6a', dim: '#2e2e2e', rgb: '200,200,200' }
...
function flicker(ctx, W, H, tSec, pal) {
  const fl = ((tSec * 90) % H) | 0
  ctx.fillStyle = `rgba(${pal.rgb},0.05)`
  ctx.fillRect(0, fl, W, 6)
}

// Generative idle terminal. `project` may be null (dark/offline CRT); `pal` is
// the screen's colour-bar palette { main, mid, dim, rgb }.
export function drawIdle(canvas, project, tSec, level, pal) {
  ...
  if (!project) {
    // Offline: rolling gray static + NO SIGNAL.
    ctx.fillStyle = GRAY.mid
  ...
  for (let r = 0; r < rows; r++) {
    ...
    ctx.fillStyle = `rgba(${pal.rgb},${fade.toFixed(2)})`
  ...
  scanlines(ctx, W, H)
  flicker(ctx, W, H, tSec, pal)
}

// Active audio visualizer. `info` = { name, sub }; `pal` is the screen palette.
export function drawViz(canvas, freq, time, info, tSec, pal) {
  const ctx = canvas.getContext('2d')
  const W = canvas.width, H = canvas.height
  ctx.fillStyle = '#050505'
  ctx.fillRect(0, 0, W, H)
